...
if __name__ == '__main__':
    ...
    G_AB_root = 'logs/cuneiform/20211101_101923/model'
    G_AB_iters = list(range(6899, 7900, 300))
    G_Glyphy_root = 'logs/cuneiform/20211101_101923/model'
    G_Glyphy_iters = list(range(6899, 7900, 300))
    A_root = 'data/cuneiform/train/A'
    input_folders = []
    valid_classes = os.listdir(A_root)
    for d in valid_classes:
        if os.path.exists(os.path.join(A_root, d)):
            input_folders.append(os.path.join(A_root, d))
    target_num = 200 #25898
    output_path = 'results/syn_database_cun_agsgan'
    ...
    for folder in input_folders:
        dataset = Folder_Dataset(folder, "*.png", transforms_)
        if len(dataset) == 0:
            continue
        ...
        while cnt_inner < target_num:
            ...
            for i, ndarr in enumerate(ndarrs):
                ...
                label = str(class_name).encode('utf-8')
